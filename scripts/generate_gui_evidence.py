...
def main() -> int:
    ...
    belief = {
        Coord(1, 1): 0.10, Coord(1, 2): 0.18, Coord(2, 1): 0.24,
        Coord(2, 2): 0.32, Coord(3, 2): 0.16,
    }
    scent = {Coord(1, 1): 0.9, Coord(1, 2): 0.62, Coord(2, 1): 0.62}
    live = LiveSnapshot(
        7, 7, Coord(4, 4), frozenset({Coord(2, 4), Coord(3, 4)}),
        scent, belief, 12, "I moved north through open ground", 518,
        "FastMCP connected", "commit acknowledged", True,
    )
    output = Path("docs/screenshots")
    render_live_png(live, output / "thief-live-local-truth.png")
    state = BoardState(
        7, 7, Coord(4, 4), Coord(2, 2),
        frozenset({Coord(2, 4), Coord(3, 4)}), 2, 12,
    )
